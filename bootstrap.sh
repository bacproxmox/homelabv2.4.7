...
REPO_URL="${REPO_URL:-https://github.com/bacproxmox/homelabv2.4.7.git}"
INSTALL_DIR="${INSTALL_DIR:-/root/homelabv2.4.7}"
BRANCH="${BRANCH:-main}"
BOOTSTRAP_MARKER="/root/.homelabv2.4.7-bootstrap-reboot-done"
export HOMELAB_VERSION="2.4.7"
...
' "$cfg" > /tmp/storage.cfg.new
  ...
  sed -i     -e 's/^btrfs: local-system$/btrfs: local/'     -e 's/^btrfs: local-btrfs$/btrfs: local/'     "$cfg"
  pvesm status || true
}
...
safe_reboot_gate() {
  if need_safe_reboot; then
    if [[ ! -f "$BOOTSTRAP_MARKER" ]]; then
      ...
      echo "bash <(curl -fsSL https://raw.githubusercontent.com/bacproxmox/homelabv2.4.7/main/bootstrap.sh)"
      echo
      sleep 8
      reboot
    else
...
echo "========================================="
echo " Homelab v2.4.7 - Bootstrap"
echo " Proxmox Subscription Prep + Repo Clone"
echo "========================================="
...
# Eski invalid apt backup dosyalarını taşı.
...
apt install -y   curl wget git sudo nano jq unzip net-tools htop ifupdown2   ca-certificates gnupg lsb-release python3 dos2unix rsync sshpass   openssh-client openssh-server iputils-ping dnsutils tar gzip   gdisk zfsutils-linux
...
echo
echo "[11/11] Cloning/updating Homelab repo..."
...
echo
echo "Starting Homelab v2.4.7 menu..."
